...
#' require(plotly) # the suggested package 'plotly' is required
#'
#' boxplot(deeplift_revcancel)
#'
#' # ------------------------- Advanced: Plotly -------------------------------
...
#' p <- plot(deeplift_revcancel, classes = 1:2) +
#'   theme_bw() +
  ...
  public = list(
    ...
    boxplot = function(boxplot_data = "all",
      ...
      private$boxplot(preprocess_FUN, boxplot_data, classes, ref_datapoint,
                      ...
                      individual_max, as_plotly, "Contribution")
    }
  ),
  private = list(
    run = function() {

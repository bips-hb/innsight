...
#'   nn_softmax(dim = 2)
#'   )
#' data <- torch_randn(15, 5)
#'
#' # Calculate LIME for the first 5 instances and set the
#' # feature and outcome names
#' lime <- LIME$new(model, data[1:5, ], data_ref = data,
...
#' # an R6 LIME object
#' lime <- run_lime(model, data[1:5, ], data_ref = data,
#'                  input_names = c("Car", "Cat", "Dog", "Plane", "Horse"),
#'                  output_names = c("Buy it!", "Don't buy it!"))
#'
...
#'                 output_names = c("Buy it!", "Don't buy it!"))
#'
#' # Calculate LIME for the first 5 instances
#' lime <- run_lime(conv, data[1:5], data_ref = data, n_perturbations = 5)
#'
#' # Plot the result for both classes
#' plot(lime, output_idx = c(1, 2))
#'

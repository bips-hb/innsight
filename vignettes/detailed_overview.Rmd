...
libraries. This property makes the package outstanding and extreme
flexible. Internally, each layer is analyzed and the relevant parameters
and components are extracted into a list. Afterwards, a customized 
...
-   In a **torch** model, the shape of the inputs is not stored; hence it
    must be specified with argument `input_dim` within the
    initialization of the `Converter` object.
    
-   An activation function can only occur directly after a linear,
    convolutional or pooling layer and not at arbitrary positions.
...
also possible to pass a self defined model in form of a named list to
the `Converter` class. This enables the interpretation of networks from
other libraries with all available methods provided by the **innsight**
package.
...
there are other optional keys that can be used to name the input
...
-   **`output_dim`** (optional)\
    ...
    compared with it to avoid errors during converting the model.
...
-   the input names are
    ...
    -   `list(c("C1", "C2", ...), c("L1", "L2", ...))` for 1D input
        layers and
    -   `list(c("C1", "C2", ...), c("H1", "H2", ...), c("W1", "W2", ...))`
        for 2D input layers.
...
-   **`input_layers`:** The list indices from the `layers` list going
    ...
    index in `layers` is the only input layer. If this layer is an input layer,
    use the value `0`.
...
-   **`output_layers`:** The list indices from the `layers` list that
    ...
    output layer. If this layer is an output layer, use the value `-1`.
...
**Note:** For this layer the argument `dim_in` is a list of input
dimensions.
...
With the argument `input_dim` the size of the input excluding the batch
...
-   **Multiple inputs:** If the passed model has multiple input layers,
    ...
    passed.\
    **Note:** With multiple input layers, it is required that the original
...
According to the shapes from the argument [`input_dim`](#argument-input_dim) the input names
for each layer and dimension can be passed with the optional argument
`input_names`. This means that for each integer in `input_dim` a vector
of this length is passed with the labels, which is then summarized for
all dimensions in a list. The labels can be provided both as normal character 
...
visualized later in Step 3. For the individual input formats, the input names
...
calculations are performed. Accepted are currently the 32-bit floating
...
accordingly into the data format `torch_float` or `torch_double`.

**Note:**\
At this point we want to note that this decision is especially crucial
...
As already described in the [Get started
vignette](https://bips-hb.github.io/innsight/articles/innsight.html), a
...
-   `input_dim`: This field is more or less a copy of the argument
    `input_dim`, only unified that it is always a list of the input
    shapes for each input layer, i.e. the argument `input_dim = c(4)`
    turns into `list(c(4))`.
    
-   `input_names`: Analog to the field `input_dim`, the field
    `input_names` contains the input labels of the argument
    `input_names`, but as a list of the label lists for each input
...
`output_names` field. Remember that in these fields, regardless of the
...
following and internally realized by the superclass `InterpretingMethod`
...
-   Base R data types like `matrix`, `array`, `data.frame` or other
    ...
    stored as a `torch_tensor` in the given `dtype` afterwards.
...
**Note:** The argument data is a necessary argument only for the local
...
`InterpretingMethod` superclass. But each method has method-specific
...
```{r, echo=FALSE, fig.cap = "**Fig. 1:** Example neural network", out.width = "80%"}
...
networks is the *Gradients* method introduced by Simonyan et al. (2013),
also known as *Vanilla Gradients* or *Silency maps*. This method
computes the gradients of the selected output with respect to the input
variables. Therefore the resulting relevance values indicate those
...
**innsight**-method `Gradient` inherits from the superclass 
`InterpretingMethod`, meaning that we need
to change the term `Method` to `Gradient`. The only model-specific
argument is `times_input`, which can be used to switch between the two
methods `Gradient` (default `FALSE`) and `GradxInput` (`TRUE`). For more
information on the method `GradxInput` see [this
subsection](#gradient-x-input-and-smoothgrad-x-input).
...
networks are highly non-linear, so this interpretation is only valid for
...
base + 
  ...
  geom_segment(aes(x = 0.45, y = -0.05, xend = 0.45, yend = 0.05), size = 0.8) +
  geom_segment(aes(x = -0.03, y = tanh(0.45), xend = 0.03, yend = tanh(0.45)), size = 0.8) +
  annotate("text", label = "x[1]", x = 0.45, y = -0.12, size = 5, parse = TRUE) +
  annotate("text", label = "f(x[1])", x = -0.13, y = tanh(0.45), size = 5, parse = TRUE) +
  geom_segment(aes(x = 0.25, xend = 0.65, y = tanh(0.45) - 0.2 / cosh(0.45)**2, 
                   yend = tanh(0.45) + 0.2 / cosh(0.45)**2), 
               color = "red", alpha = 0.7, size = 1.5) +
  geom_point(data = data.frame(x = 0.45, y = tanh(0.45)), 
             mapping = aes(x = x, y = y))
```
...
The *SmoothGrad* method, introduced by Smilkov et al. 2017, addresses a
...
`SmoothGrad` inherits from the superclass `InterpretingMethod`, meaning
that we need to change the term `Method` to `SmoothGrad`. In addition,
there are the following three model-specific arguments:
...
-   `times_input` (default: `FALSE`): Similar to the `Gradient` method,
    ...
    `SmoothGrad` (`FALSE`) and `SmoothGradxInput` (`TRUE`). For more
    information on the method `SmoothGradxInput` see [this
    subsection](#gradient-x-input-and-smoothgrad-x-input).
...
fig <- base +
  geom_function(fun = func, alpha = 0.7) +
  geom_segment(aes(x = 0.6, y = -0.05, xend = 0.6, yend = 0.05), size = 0.8) +
  geom_segment(aes(x = -0.03, y = func(0.6), xend = 0.03, yend = func(0.6)), size = 0.8) +
...
data <- data.frame(x = x - 0.4/norm, xend = x + 0.4 / norm,
                   y = y - grad * 0.4 / norm, yend = y + grad * 0.4 / norm)
mean_grad <- data.frame(x = x0 - 0.6/ norm_mean, xend = x0 + 0.6 / norm_mean,
                        y = y0 - grad_mean * 0.6 / norm_mean, 
                        yend = y0 + grad_mean * 0.6 / norm_mean)

fig + 
  geom_segment(data = data, mapping = aes(x = x, xend = xend, y = y, yend = yend),
                color = "darkblue", alpha = 0.3, size = 0.5) +
  geom_segment(data = mean_grad, mapping = aes(x = x, xend = xend, y = y, yend = yend),
                color = "red", alpha = 0.9, size = 1.25) +
  geom_function(fun = func, alpha = 0.7) +
  geom_point(mapping = aes(x = x, y = y), color = "blue", size = 0.8) +
  geom_point(data = data.frame(x = 0.6, y = func(0.6)), 
             mapping = aes(x = x, y = y))
```
...
### Gradient x Input and SmoothGrad x Input

The methods *Gradient x Input* and *SmoothGrad x Input* are as simple as
they sound: the gradients are calculated as in the [gradient
section](#method-vanilla-gradient) and then multiplied by the respective
input. They were introduced by Shrikumar et al. 2016 and have a
well-grounded mathematical background despite their simple idea. The basic
idea is to decompose the output according to its relevance to each input
variable, i.e. we get variable-wise additive effects 
$$
\tag{1}
...
guaranteed anymore. The *Gradient x Input* method now considers the data
point $x$ and sets $z = 0$. In addition, the residual term and the
summand $f(0)$ are ignored, which then results in the following
approximation of $f(x)$ in variable-wise relevances

$$
f(x)_c \approx \sum_{i = 1}^n \frac{\partial\ f(x)_c}{\partial\ x_i} \cdot x_i, \quad \text{hence}\\
\text{Grad x Input}(x)_i^c = \frac{\partial\ f(x)_c}{\partial\ x_i} \cdot x_i.
$$
...
$$
\begin{align}
&f(z) = f(x) + \sum_{i = 1}^n \frac{\partial\ f(x)}{\partial\ x_i} (z_i - x_i) + \varepsilon(f, z, x)\\
\Leftrightarrow\quad & f(x) = f(z) - \sum_{i = 1}^n \frac{\partial\ f(x)}{\partial\ x_i} (z_i - x_i) - \varepsilon(f, z, x)\\
\Leftrightarrow\quad & f(x) = f(z) + \sum_{i = 1}^n \frac{\partial\ f(x)}{\partial\ x_i} (x_i - z_i) - \varepsilon(f, z, x)
\end{align}
$$ 
...
$$
\begin{align}
f(x) &= f(0) + \sum_{i = 1}^n \frac{\partial\ f(x)}{\partial\ x_i} x_i - \varepsilon(f, z, x) \tag{3}\\
&\approx \sum_{i = 1}^n \frac{\partial\ f(x)}{\partial\ x_i} x_i
\end{align}
$$
...
local fluctuations: 
$$
\text{SmoothGrad x Input}(x)_i^c = \frac{1}{K} \sum_{j = 1}^K \frac{\partial\ f(x + \varepsilon^j)_c}{\partial\ x_i + \varepsilon_i^j} \cdot (x_i + \varepsilon_i^j),\quad \varepsilon^1, \ldots, \varepsilon^K \sim \mathcal{N}(0,\sigma^2).
$$
...
arguments. These variants can be chosen with the argument `times_input`:
...
*Gradient x Input* method, which indeed deviates from the actual value
only by the negative of the remainder term at position $0$.

```{r, echo = FALSE, fig.width=5, fig.height= 3, fig.cap= "**Fig. 4:** Gradient x Input method"}
base +
  geom_function(fun = func, alpha = 0.7) +
  geom_segment(aes(xend = 0, yend = 0.5*(c(func(0.49)) - grad_func(0.49) * 0.49) , x = 0.75, y = 0.25), color = "black", size = 0.25, arrow = arrow(length = unit(0.2,"cm"), type = "closed"), alpha = 0.8)  +
  annotate("text", label = "ε(f,0.49,0)", x = 0.85, y = 0.25) +
  geom_segment(aes(x = 0.49, y = func(0.49), xend = 0.49, yend = grad_func(0.49) * 0.49), color = "red", size = 0.3) +
  geom_segment(aes(x = 0, y = 0 , xend = 0, yend = c(func(0.49)) - grad_func(0.49) * 0.49), color = "red", size = 0.3) +
  geom_segment(aes(xend = 0.49, yend = c(func(0.49)) - 0.5 * (c(func(0.49)) - grad_func(0.49) * 0.49), x = 0.75, y = 0.25), color = "black", size = 0.25, arrow = arrow(length = unit(0.2,"cm"), type = "closed"), alpha = 0.8) +
  geom_function(fun = function(x) grad_func(0.49) *(x - 0.49) + c(func(0.49)), color = "red", alpha = 0.7, xlim = c(-0.2,1.1)) +
  geom_segment(aes(x = 0.49, y = -0.05, xend = 0.49, yend = 0.05), size = 0.8) +
  geom_segment(aes(x = -0.03, y = func(0.49), xend = 0.03, yend = func(0.49)), size = 0.8) +
  annotate("text", label = "x[1]", x = 0.49, y = -0.12, size = 6, parse = TRUE) +
  annotate("text", label = "f(x[1])", x = -0.13, y = func(0.49), size = 6, parse = TRUE) +
  geom_point(data = data.frame(x = 0.49, y = grad_func(0.49) * 0.49), 
             mapping = aes(x = x, y = y), color = "red", alpha = 0.7) +
  geom_point(data = data.frame(x = 0.49, y = func(0.49)), 
...
It is also possible to use the **SmoothGrad** method to perturb the input
$x_1 = 0.49$ a bit and return an average value of the *Gradient x Input*
method. Figure 5 shows the individual linear approximations of the
first-order Taylors for the Gaussian perturbed copies of $x_1$, and the
blue dots describe the respective Gradient x Input values. The red dot
represents the mean value, i.e. the value of the *SmoothGrad x Input*
method at $x_1 = 0.49$. 

```{r, echo = FALSE, fig.width=5, fig.height= 3, fig.cap= "**Fig. 5:** SmoothGrad x Input method"}
set.seed(111)
...
base +
  ...
  annotate("text", label = "x[1]", x = 0.49, y = -0.12, size = 6, parse = TRUE) +
  annotate("text", label = "f(x[1])", x = -0.13, y = func(0.49), size = 6, parse = TRUE) +
  geom_function(fun = function(z) m[1] * z + b[1], color = "blue", alpha = 0.3, na.rm = TRUE, size = 0.3) +
  geom_function(fun = function(z) m[2] * z + b[2], color = "blue", alpha = 0.3, na.rm = TRUE, size = 0.3) +
  geom_function(fun = function(z) m[3] * z + b[3], color = "blue", alpha = 0.3, na.rm = TRUE, size = 0.3) +
  geom_function(fun = function(z) m[4] * z + b[4], color = "blue", alpha = 0.3, na.rm = TRUE, size = 0.3) +
  geom_function(fun = function(z) m[5] * z + b[5], color = "blue", alpha = 0.3, na.rm = TRUE, size = 0.3) +
  geom_function(fun = function(z) m[6] * z + b[6], color = "blue", alpha = 0.3, na.rm = TRUE, size = 0.3) +
  geom_function(fun = function(z) m[7] * z + b[7], color = "blue", alpha = 0.3, na.rm = TRUE, size = 0.3) +
  geom_function(fun = function(z) m[8] * z + b[8], color = "blue", alpha = 0.3, na.rm = TRUE, size = 0.3) +
  geom_function(fun = function(z) m[9] * z + b[9], color = "blue", alpha = 0.3, na.rm = TRUE, size = 0.3) +
  geom_function(fun = function(z) m[10] * z + b[10], color = "blue", alpha = 0.3, na.rm = TRUE, size = 0.3) + 
  geom_point(data = data.frame(x = x, y = func(x)), mapping = aes(x = x, y = y), color = "black",
             alpha = 0.3, size = 0.8) + 
  geom_point(data = data.frame(x = x, y = m * x), mapping = aes(x = x, y = y), color = "blue",
             alpha = 0.3) +
...
The *LRP* method was first introduced by Bach et al. 2015 and has a
similar goal to the *Gradient x Input* method explained in the last
...
frequently used rules are defined ($i$ is an index of a node in layer
...
**Note:** At this point, it must be mentioned that the LRP variants do not
...
::: column-right
```{r, echo=FALSE, fig.cap = "**Fig. 6:** Layerwise Relevance Propagation"}
knitr::include_graphics("images/lrp.png")
```
:::

Analogous to the previous methods, the **innsight** method `LRP` inherits 
from the `InterpretingMetod` superclass and thus all arguments. In addition,
...
-   `rule_name` (default: `"simple"`): This argument can be used to
    ...
    use the default rule. You can set the rule for the following layer types:
...
-   `rule_param`: The meaning of this argument depends on the selected
    ...
    to the argument `rule_name`, this can also be a named list that assigns 
    a rule parameter to each layer type.
...
was about $0.3889$ when approximated with the *Gradient x Input* method. For
...
data <- matrix(
  c(0.49, # only neuron without bias term is activated
...
Shrikumar et al. in 2017. It behaves similarly to *LRP* in a
layer-by-layer backpropagation fashion from a selected output node back
to the input variables. However, it incorporates a reference value
$\tilde{x}$ to compare the relevances with each other. Hence, the
relevances of *DeepLift* represent the relative effect of the outputs of
the instance to be explained $f(x)_c$ and the output of the reference
value $f(\tilde{x})_c$, i.e. $f(x)_c - f(\tilde{x})_c$. This difference
eliminates the bias term in the relevance messages so that no more
relevance is absorbed and we have an exact variable-wise decomposition
of $\Delta y = f(x)_c - f(\tilde{x})_c$. In addition, the authors presented
two rules to propagate relevances through the activation part of the
individual layers, namely **rescale** and **reveal-cancel** rule.
The rescale rule simply scales the contribution to the difference from 
reference output according to the value of the
activation function. The reveal-cancel rule considers the average impact 
after adding the negative or positive contribution revealing dependencies 
...
deeplift <- DeepLift$new(conv, iris[, -5], x_ref = x_ref, ignore_last_act = FALSE,
...
One of the earliest methods specifically for neural networks was the *Connection Weights* method invented by Olden et al. in 2004, resulting in a global relevance score for each input variable. The basic idea of this approach is to multiply all path weights for each possible connection between an input variable and the output node and then calculate the sum of all of them. However, this method ignores all bias vectors and all activation functions during calculation. Since only the weights are used, this method is independent of input data and, thus, a global interpretation method. In this package, we extended this method to a local one inspired by the method *Gradient x Input* (see [here](#gradient-x-input-and-smoothgrad-x-input)). Hence, the local variant is simply the point-wise product of the global *Connection Weights* method and the input data. You can use this variant by setting the `times_input` argument to `TRUE` and providing input data.

The method **innsight** method `ConnectionWeights` also inherits from the 
superclass `InterpretingMethod`, meaning that you need to change the 
term `Method` to `ConnectionWeights`. The only model-specific argument is 
`times_input`, which can be used to switch between the
global (`FALSE`) and the local (`TRUE`) *Connection Weights* method.
...
as S3 methods (`get_result()`, `plot()` and `boxplot()`) for easier handling.
...
# Create converter
tab_conv <- Converter$new(tab_model, input_dim = c(4), 
...
name as an character for argument `type`. As mentioned before, there is also
...
input and/or output layers. In these cases, a list (or a list of lists) with 
...
tab_grad <- Gradient$new(tab_conv, tab_data, output_idx = c(1,3),
                         times_input = TRUE)
# Get result
...
conv <- Converter$new(model)
data <- lapply(list(c(10,10,2), c(11)), 
...
grad <- Gradient$new(conv, data, output_idx = list(c(1,2), c(1,3)), 
...
Another practical way to output the results, for example to visualize them 
...
* `'feature'`: Feature represents the input names of the corresponding input 
...
As already mentioned, the columns for the channel and for the image width 
are then added:
...
# calculate mean absolut gradient
df <- aggregate(df$value, 
                by = list(feature = df$feature, class = df$output_node),
                FUN = function(x) mean(abs(x)))

ggplot(df) +
  geom_bar(aes(x = feature, y = x, fill = class), stat = "identity",
           position = "dodge") +
  ggtitle("Mean over absolut values of the gradients") +
  xlab("Input feature") + ylab("Mean(abs(gradients))") +
  theme_bw()
```
</details>
...
This method visualizes the result of the selected method and enables a visual 
in-depth investigation with the help of the S4 classes `innsight_ggplot2` and
...
nodes or classes for the plot can be selected with the argument `output_idx`. 
But this has to be a subset of the [argument `output_idx`](#argument-output_idx) 
passed to the respective method previously, because only for these outputs were
the results calculated. The different results for the selected data points and 
...
the given data.  
**Note:** This argument will be ignored for the global *Connection 
Weights* method.
...
channels is used.  
**Note:** This argument is used only for 1D signal and 2D image input data.
...
**Note:** Make sure that the suggested package **plotly** is installed in 
your R session.

```{r, eval = FALSE}
# Class method
method$plot(data_idx = 1, 
            output_idx = NULL, 
            aggr_channels = "sum",
            as_plotly = FALSE)
...
print("Due to the size, this image was not generated in the R vignette! See the innsight webpage for this image: https://bips-hb.github.io/innsight/articles/detailed_overview.html#plot-single-results-plot")
```
...
p <- plot(img_grad, output_idx = c(1,2), data_idx = c(1,4),
          aggr_channels = "norm")
...
## Plot summarized results (`boxplot`)

This method visualizes summarized results of the selected method over all data
points (or a subset) and enables a visual in-depth investigation with the help 
...
`output_idx` to select the individual output nodes for the plot. But this 
has to be a subset of the [argument `output_idx`](#argument-output_idx) 
passed to the respective method previously, because only for these outputs were
the results calculated. For tabular and 1D signal data, boxplots are created in 
...
subset of them by passing the indices. E.g. with `c(1:10, 25, 26)` only the 
...
**Note:** Because of the complexity of 2D inputs, this argument is used only 
for tabular and 1D signal inputs and disregarded for 2D image inputs.
...
data points in a dropdown menu, which are drawn in individually analogous to 
`ref_data_idx` only for more data points. With the default value `NULL` the 
first `individual_max` data points are used.  
**Note:** If `ref_data_idx` is specified, this data point will be added to 
...
```{r, eval = FALSE}
# Class method
method$boxplot(output_idx = NULL,
               data_idx = "all",
               ref_data_idx = NULL,
               aggr_channels = "sum",
               preprocess_FUN = abs,
...
```{r, fig.width = 8, fig.height=4, echo = TRUE, eval = FALSE}
# You can do the same with the plotly-based plots
p <- boxplot(tab_grad, output_idx = c(1,3), data_idx = 1:50,
...
```
```{r, eval=Sys.getenv("RENDER_PLOTLY", unset = 0) == 0, echo = FALSE}
print("Due to the size, this image was not generated in the R vignette! See the innsight webpage for this image: https://bips-hb.github.io/innsight/articles/detailed_overview.html#plot-summarized-results-boxplot")
```
```{r, fig.width = 8, fig.height=4, echo = FALSE, message=FALSE, eval=Sys.getenv("RENDER_PLOTLY", unset = 0) == 1}
# You can do the same with the plotly-based plots
p <- boxplot(tab_grad, output_idx = c(1,3), data_idx = 1:50,
...
In the proceeding two sections, the basic `plot` and `boxplot` functions have 
...
example, two plots with different scales for each output node need to be 
...
keras_model_concat <- keras_model(inputs = input_image, 
...
keras_model_concat <- keras_model(inputs = list(input_image, input_tab), 
...
All **ggplot2**-based plots generated by the **innsight** package are object of the
S4 class `innsight_ggplto2`, which is a simple extension of a ggplot2 object
enabling a more detailed analysis and comparison of the results. In addition,
it provides a way to visualize the results of models with multiple 
input or output layers with different scales, e.g. images and tabular data. The values 
in the slots of this class depend very much on whether results for multiple input or output
layers (`multiplot = TRUE`) or for only a single input and output layer 
(`multiplot = FALSE`) are visualized, which is described in the following. But 
in general, however, we would like to point out that the slots are only explained here for 
internal purposes and should not be changed in standard use cases. 
...
# Now we can add geoms, themes and scales as usual for ggplo2 objects
df <- data.frame(x = c(0.5,  0.5, 10.5, 10.5, 0.5), 
...
# In this case, 'grobs' is a 2x4 matrix
p@grobs
...
upper case (see there for details and examples).
</li>
<li>
`plot`, `print` and `show` : These methods provide generic visualization 
functions, which all behave the same in this case. They create the plot of the
...
<li>
`[<-` : Since in this case each individual plot from the slot `grobs` 
represents an independent plot and these are joined with `arrageGrob`, 
each of the plots can also be changed individually and is independent of the 
others. Individual modifications can be made with this generic function.
<details>
<summary> Examples </summary>
...
* Due to the size of interactive plots, the following ploty-based plots are 
...
*(num_datapoints, num_outputnodes $\cdot$ num_inputs)*.
...
<ul>
<li>
`[` : With this basic generic indexing functions, the plots of individual 
...
p
```
```{r, eval=Sys.getenv("RENDER_PLOTLY", unset = 0) == 1, echo = FALSE, fig.height=4,out.width= "100%", fig.width=20}
# Create plot
p <- plot(res_two_inputs, output_idx = list(1,2), data_idx = c(1,3), 
          as_plotly = TRUE)
...
p_new
```
```{r, eval=Sys.getenv("RENDER_PLOTLY", unset = 0) == 1, echo = FALSE, fig.height=5, fig.width=10}
# Now you can select specific rows and columns for in-depth investigation,
# e.g. only the result for output "Y2"
p_new <- p[1:2, 3:4]
...
<li>
`[[` : With this basic generic indexing functions, you can select a single plot 
...
```
```{r, fig.height=4, fig.width=10, echo = FALSE, eval=Sys.getenv("RENDER_PLOTLY", unset = 0) == 1}
# Create plot
p <- plot(res_two_inputs, output_idx = list(1,2), data_idx = 1,
          as_plotly = TRUE)
...
# You can also pass additional arguments to the method 'plotly::subplot',
# e.g. the margins
plotly::config(print(p, margin = c(0.03, 0.03, 0.01, 0.01), 

...
knitr::opts_chunk$set(
  ...
  fig.align = "center",
  out.width = "90%",
  eval = requireNamespace("palmerpenguins") & requireNamespace("luz") & torch::torch_is_installed()
  )
```
...
overview of the data, let's create a pair plot of some variables using 
**GGally**:
...
# create plot
GGally::ggpairs(penguin_data, ggplot2::aes(color = species, alpha = 0.75), 
                columns = 3:7,
                progress = FALSE)
```
...
classification. In the following, we will see if these hypothesis match 
...
models trained in **luz** with the package **innsight**. 
...
fitted <- net %>%
  setup(
    loss = function(input, target) nnf_nll_loss(log(input), target, weight = weight),
...
Analogous to the first example with the iris dataset, we can easily create the `Converter` object by passing the extracted model and the input dimension to 
the R6 class.
...
explained in the other more [detailed vignette](https://bips-hb.github.io/innsight/vignette/detailed_overview.html#step-2-apply-selected-method). 
...
# data to be analyzed (in this case, we use the validation set)
data <- test_ds$x
...
This is an Adelie penguin, and it shows exactly what we thought at the 
...
it gives by far the highest positive relevance for the class Adelie and a very 
...
Adelie penguins were sighted  on all three islands equally. For this reason, 
...
boxplot(lrp_ab_2, output_idx = c(1,3), preprocess_FUN = identity,
        ref_data_idx = 1) +
  coord_flip() +

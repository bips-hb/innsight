...
method and this requires a successful installation of LibTorch and other 
dependencies (`torch::install_torch()`), no examples can be run in the R 
vignette for memory reasons. For the executed codechunks we refer to [our website](https://bips-hb.github.io/innsight/articles/Example_2_penguin.html).
...
it is visible that the Gentoo species (blue) is much heavier and has longer 
...
defined as `nn_module` and the data as a `torch::dataset`, but currently 
...
explained in the other more [detailed vignette](https://bips-hb.github.io/innsight/articles/detailed_overview.html#step-2-apply-selected-method). 
...
data in advance (e.g. the absolute value).

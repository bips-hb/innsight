...
need gradient-based explanations.
...
element represents the sensitivity of an output to an input feature.
...
This provides a decomposition of the output into feature-wise contributions:
...
- You need other methods like LRP or DeepLift
- You want visualization capabilities (`plot`, `plot_global`)
- You need to work with the unified `innsight` interface
...
Use `return_object = TRUE` to get a `TorchGradientResult` object:
...
The `TorchGradientResult` object provides the same interface as converter-based
methods:

```{r object_methods}
# Get results in different formats
result_array <- result_obj$get_result("array")
result_tensor <- result_obj$get_result("torch_tensor")
result_df <- result_obj$get_result("data.frame")
...
### Comparison: Tensor vs Object

Choose the approach that fits your workflow:

```{r comparison_workflow, eval=FALSE}
# Workflow 1: Raw tensors (fast, minimal)
grads <- torch_grad(model, data)
# Work directly with torch operations
important_features <- grads[1, , 1]$abs()$topk(3)

# Workflow 2: Objects (feature-rich)
result <- torch_grad(model, data, return_object = TRUE)
# Use high-level methods
plot(result)
summary_stats <- result$get_result("data.frame")
```

Both approaches produce identical numerical results - choose based on your needs.
...
For comprehensive neural network interpretation including non-gradient methods

...
test_that("ConnectionWeights (local): Dense-Net", {
  ...
  expect_true(inherits(box, "ggplot"))
  expect_warning(box <- boxplot(cw_first, as_plotly = TRUE))
  expect_true(inherits(box, "plotly"))
})
...
test_that("ConnectionWeights (global): Keras model with two inputs + two outputs", {
  ...
  # Test plot function with channels first
  expect_warning(p <- plot(cw_first))
  expect_true(inherits(p, "ggplot"))
  expect_warning(p <- plot(cw_first, output_idx = list(c(2), c(1))))
  expect_true(inherits(p, "ggplot"))
  expect_warning(p <- plot(cw_first, as_plotly = TRUE))
  expect_true(inherits(p, "plotly"))
  expect_warning(p <- plot(cw_first, as_plotly = TRUE,
                           output_idx = list(c(2), c(1))))
  expect_true(inherits(p, "plotly"))


  # Test plot function with channels last
  expect_warning(p <- plot(cw_last))
  expect_true(inherits(p, "ggplot"))
  expect_warning(p <- plot(cw_last, output_idx = list(c(2), c(1))))
  expect_true(inherits(p, "ggplot"))
  expect_warning(p <- plot(cw_last, as_plotly = TRUE))
  expect_true(inherits(p, "plotly"))
  expect_warning(p <- plot(cw_last, as_plotly = TRUE,
                           output_idx = list(c(2), c(1))))
  expect_true(inherits(p, "plotly"))
  ...
  # Test boxplot
  expect_error(boxplot(cw_first))
  expect_error(boxplot(cw_first, as_plotly = TRUE))
})

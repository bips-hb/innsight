...
test_that("Test initialization and forward for conv2d_layer", {
  ...
  W <- torch_randn(out_channels, in_channels, kernel_height, kernel_width, dtype = torch_double())
...
test_that("Test get_pos_and_neg_outputs and get_gradient", {
  ...
  expect_lt(as_array(mean((out_stride$pos + out_stride$neg - y_true)^2)), 1e-12)

  # Test get_gradient
  sum(y_true)$backward()
  for (model_out in 1:3) {
    grad_stride <- conv2d_stride$get_gradient(torch_ones(c(dim(out_stride$pos), model_out), dtype = torch_double()), W)
  ...
  # Test get_pos_and_neg_outputs
  out_dilation <- conv2d_dilation$get_pos_and_neg_outputs(input, use_bias = TRUE)
  expect_equal(dim(out_dilation$pos), dim(y_true))
  expect_equal(dim(out_dilation$neg), dim(y_true))
  expect_lt(as_array(mean((out_dilation$pos + out_dilation$neg - y_true)^2)), 1e-12)

  # Test get_gradient
  sum(y_true)$backward()
  for (model_out in 1:3) {
    grad_dilation <- conv2d_dilation$get_gradient(torch_ones(c(dim(out_dilation$pos), model_out), dtype = torch_double()), W)
...
test_that("Test get_input_relevances", {
  ...
  for (model_out in c(1,2,4)) {
    rel <- torch_randn(batch_size,out_channels, out_length_stride[1], out_length_stride[2], model_out, dtype = torch_double())
    rel_true <- sum(rel, dim = 2:4)

    # Simple rule
    rel_simple_stride <- conv2d_stride$get_input_relevances(rel)
    expect_equal(dim(rel_simple_stride), c(batch_size, in_channels, in_height, in_width, model_out))
    rel_simple_stride_sum <- sum(rel_simple_stride, dim = 2:4)
    expect_lt(as_array(mean((rel_true - rel_simple_stride_sum)^2)), 1e-12)

    # Epsilon rule
    rel_epsilon_stride <- conv2d_stride$get_input_relevances(rel, rule_name = "epsilon")
    expect_equal(dim(rel_epsilon_stride), c(batch_size, in_channels, in_height, in_width, model_out))

    # Alpha-Beta rule
    rel_alpha_beta_stride <- conv2d_stride$get_input_relevances(rel, rule_name = "alpha_beta")
    expect_equal(dim(rel_alpha_beta_stride), c(batch_size, in_channels, in_height, in_width, model_out))
  }
  ...
  # Test get_input_relevances
  for (model_out in c(1,2,4)) {
    rel <- torch_randn(batch_size,out_channels, out_length_dilation[1], out_length_dilation[2], model_out, dtype = torch_double())
    rel_true <- sum(rel, dim = 2:4)

    # Simple rule
    rel_simple_dilation <- conv2d_dilation$get_input_relevances(rel)
    expect_equal(dim(rel_simple_dilation), c(batch_size, in_channels, in_height, in_width, model_out))
    rel_simple_dilation_sum <- sum(rel_simple_dilation, dim = 2:4)
    expect_lt(as_array(mean((rel_true - rel_simple_dilation_sum)^2)), 1e-12)

    # Epsilon rule
    rel_epsilon_dilation <- conv2d_dilation$get_input_relevances(rel, rule_name = "epsilon")
    expect_equal(dim(rel_epsilon_dilation), c(batch_size, in_channels, in_height, in_width, model_out))

    # Alpha-Beta rule
    rel_alpha_beta_dilation <- conv2d_dilation$get_input_relevances(rel, rule_name = "alpha_beta")
    expect_equal(dim(rel_alpha_beta_dilation), c(batch_size, in_channels, in_height, in_width, model_out))
  }
})
...
test_that("Test get_input_multiplier", {
  ...
  for (model_out in c(1,2,4)) {
    mult <- torch_randn(batch_size,out_channels, out_length_stride[1], out_length_stride[2], model_out, dtype = torch_double())
    ...
    expect_equal(dim(mult_rescale_stride), c(batch_size, in_channels, in_height, in_width, model_out))
    expect_lt(as_array(mean((contrib_rescale_stride - contrib_true)^2)), 1e-12)

    # Reveal-Cancel rule
    mult_revcancel_stride <- conv2d_stride$get_input_multiplier(mult, rule_name = "reveal_cancel")
    contrib_revcancel_stride <- sum(mult_revcancel_stride * diff_input, dim = 2:4)
    expect_equal(dim(mult_revcancel_stride), c(batch_size, in_channels, in_height, in_width, model_out))
    expect_lt(as_array(mean((contrib_revcancel_stride - contrib_true)^2)), 1e-12)
  }
  ...
  for (model_out in c(1,2,4)) {
    mult <- torch_randn(batch_size,out_channels, out_length_dilation[1], out_length_dilation[2], model_out, dtype = torch_double())
    contrib_true <- sum(diff_output * mult, dim = 2:4)

    # Rescale rule
    mult_rescale_dilation <- conv2d_dilation$get_input_multiplier(mult)
    contrib_rescale_dilation <- sum(mult_rescale_dilation * diff_input, dim = 2:4)
    expect_equal(dim(mult_rescale_dilation), c(batch_size, in_channels, in_height, in_width, model_out))
    expect_lt(as_array(mean((contrib_rescale_dilation - contrib_true)^2)), 1e-12)

    # Reveal-Cancel rule
    mult_revcancel_dilation <- conv2d_dilation$get_input_multiplier(mult, rule_name = "reveal_cancel")
    contrib_revcancel_dilation <- sum(mult_revcancel_dilation * diff_input, dim = 2:4)
    expect_equal(dim(mult_revcancel_dilation), c(batch_size, in_channels, in_height, in_width, model_out))
    expect_lt(as_array(mean((contrib_revcancel_dilation - contrib_true)^2)), 1e-12)
  }
})

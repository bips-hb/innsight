...
test_that("Test DeepLift method",{
  ...
  expect_error(DeepLift(NULL, iris[1:10, -5], out_class = 1))
  expect_error(DeepLift(NULL, iris[1:10, -5], out_class = 10))
  expect_error(DeepLift(analyzer, out_class = 10))
  expect_error(DeepLift(analyzer, out_class = 1.4))
  expect_error(DeepLift(analyzer, out_class = -2))
  ...
  for (rule in c("rescale", "revealcancel")) {
    result <- DeepLift(analyzer, iris[1:10,-5], x_ref = NULL, rule_name = rule, out_class = NULL)
    ...
    result <- DeepLift(analyzer, iris[1:10, -5], rule_name = rule, out_class = 1)
    expect_true("DeepLift" %in% class(result))
    expect_true("ggplot" %in% class(plot(result)))
    expect_true("ggplot" %in% class(plot(result, rank = TRUE)))
    expect_true("ggplot" %in% class(plot(result, scale = TRUE)))
  ...
  dimnames(d) <- c()
  for (rule in c("rescale", "revealcancel")) {
    result <- DeepLift(analyzer, d, x_ref = NULL, rule_name = rule, out_class = NULL)
    expect_true("DeepLift" %in% class(result))
    expect_true("ggplot" %in% class(plot(result)))
    expect_true("ggplot" %in% class(plot(result, rank = TRUE)))
    expect_true("ggplot" %in% class(plot(result, scale = TRUE)))

    result <- DeepLift(analyzer, d, x_ref = NULL, rule_name = rule, out_class = 1)
    expect_true("DeepLift" %in% class(result))
    expect_true("ggplot" %in% class(plot(result)))
    expect_true("ggplot" %in% class(plot(result, rank = TRUE)))
    expect_true("ggplot" %in% class(plot(result, scale = TRUE)))
